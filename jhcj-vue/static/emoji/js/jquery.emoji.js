...
(function ($, window, document) {
    ...
    Plugin.prototype = {
        _init: function () {
            ...
            var emoji_content = '<div class="emoji_content" style="padding:1px">';
            ...
            for (var i = 0; i < groupLength; i++) {
                ...
                for (var j = 1; j <= maxNum; j++) {
                    ...
                    panel += '<li style="width:' + imgWidth + 'px;' + 'height:' + imgWidth + 'px;"' + '><a data-emoji_code="' + notation + '" data-index="' + index + '" title="' + (title && title[j] ? title[j] : '') + '"><img src="' + path + j + file + '"/></a></li>';
                    index++;
                }
                panel += '</ul></div>';
                emoji_content += panel;
            ...
            btnTop = window.innerHeight - 278;//$(btn).offset().top + $(btn).outerHeight() + 5;
            btnLeft = 0;//$(btn).offset().left;
            $('#emoji_container_' + ix).css({'top': btnTop + 'px', 'left': btnLeft + 'px','width': '100%', 'height': 278 + 'px'});
